...
// ─── Day-Cache (Version wie dashboard-server.js / usage-scan-roots) ───
...
// ─── Limit-Erkennung (gleiche Heuristik wie scripts/dashboard-server.js) ───
...
/** Gleiche Heuristik wie scripts/dashboard-server.js (JSONL-Zeilen). */
...
// Alle .jsonl wie das Dashboard: ~/.claude/projects + CLAUDE_USAGE_EXTRA_BASES
var collectedFiles = usageScanRoots.collectTaggedJsonlFiles();
var allFiles = collectedFiles.tagged;
...
for (var fi = 0; fi < allFiles.length; fi++) {
  ...
  try {
    forEachJsonlLineSync(f, function(line) {
      ...
      if (rec) {
        var tsSig = rec.timestamp || '';
        if (tsSig.length >= 10) {
          var daySig = tsSig.slice(0, 10);
          if (!onlyToday || daySig === onlyToday) {
            var sigTags = classifyJsonlSessionSignals(line, rec);
            if (sigTags.length) {
              if (!daily[daySig]) daily[daySig] = emptyDailyBucket();
              bumpSessionSignals(daily[daySig], sigTags);
            }
          }
        }
      }
      if (line.length > 10 && scanLineHitLimit(line)) {
        try {
          var parsed = rec || JSON.parse(line);
          var lts = parsed.timestamp || '';
          if (lts.length >= 10) {
            var lday = lts.slice(0, 10);
            if (onlyToday && lday !== onlyToday) return;
      ...
      try {
        ...
        if (onlyToday && msgDay !== onlyToday) return;
        messages.push({
          ts: ts,
          day: msgDay,
...
function sigFmt(r) {
  var s = r.session_signals || emptySessionSignals();
...
console.log('');
console.log(
  '  Datum      | Output    | Cache Read | Cache:Out | Calls | Akt.h | Sig c/r/y/i | Limit?',
);
console.log(
  '  -----------|-----------|------------|-----------|-------|-------|-------------|----------------',
);
...
for (var di = 0; di < days.length; di++) {
  ...
  var rVis = daily[d];
  var sVis = rVis.session_signals || emptySessionSignals();
  if ((sVis.interrupt || 0) + (sVis.retry || 0) > 0) {
    label += ' \u00b7 int:' + (sVis.interrupt || 0) + ' retry:' + (sVis.retry || 0);
  }
...
console.log(
  '  Zusatz: int/retry = Session-Signale (JSONL-Heuristik, gleiches Schema wie Dashboard).',
);
console.log('');
